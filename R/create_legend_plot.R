create_legend_plot <- function(df, n_legend_items){
  ...
  df <- df %>%
    arrange(.data$skill_name)

  # Create a data frame for the legend
  legend_df <- df  %>%
    ungroup() %>%
    select(.data$skill_name, .data$color) %>%
    distinct(.data$skill_name, .data$color)


  # Generate a custom legend using ggplot
  legend_plot <- ggplot(legend_df, aes(x = 1, y = .data$skill_name, fill = .data$color)) +
    ...
    geom_text(aes(label = .data$skill_name), hjust = 0, color = "black", nudge_x = -1.4*n_legend_items/11-0.02, nudge_y = -0.02) +
    geom_text(aes(label = .data$skill_name), hjust = 0, color = "white", nudge_x = -1.4*n_legend_items/11) +
    scale_fill_identity() +
    coord_fixed() +
    theme_void() +
    theme(
