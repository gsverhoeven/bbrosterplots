...
```{r unpack, include=FALSE}
# unpack parameters

if(is.null(params$group_name)){
  group_name <- "my_group"
} else {
group_name <- params$group_name
}

if(is.null(params$tournament_ruleset)){
  tournament_ruleset <- "my_ruleset"
} else {
...
if(is.null(params$cover_image)){
  params$cover_image <- system.file("extdata", "default_cover.jpg", package = "bbrosterplots")
}
if(is.null(params$scale_cover_perc)){
  params$scale_cover_perc = "100%"
}

if(is.null(params$perc)){
  params$perc <- FALSE
} 

if(is.null(params$team)){
  params$team <- FALSE
} 
```
